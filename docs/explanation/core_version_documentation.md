...
To force the energy system to decrease its emissions, two levers can constrain the annual emissions: the equation imposes a maximum yearly emissions threshold on the GWP ($gwp_{limit}$); and the equation fixes the minimum renewable primary energy share.
