...
and can solve a basic optimization problem using the Gurobi solver,
...
def run_test():
    ...
    """
    print("="*70)
    print("Testing PyOptInterface with Gurobi (Documentation-based)")
    print("="*70)

    try:
        # 1. Create a model using the gurobi-specific backend
        model = gurobi.Model()
        ...
        if term_status == poi.TerminationStatusCode.OPTIMAL:
            ...
            print("\n✓ PyOptInterface and Gurobi are working correctly.")
        else:
            print("\n✗ Could not find the optimal solution.")

    except Exception as e:
        print(f"\n✗ An error occurred: {e}")
        print("  Please ensure Gurobi is correctly installed and licensed.")
        print("  Note: The 'pyoptinterface' Gurobi backend may require the 'gurobipy' package.")
