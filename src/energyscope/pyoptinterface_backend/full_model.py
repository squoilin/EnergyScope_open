...
This implementation matches the AMPL model structure and includes all constraints
from Equations 2.1-2.39 with optimizations for computational efficiency.
"""
...
def build_full_model(data, solver='gurobi', verbose=True, enable_output=True, timing=True):
    ...
    data : dict
        ...
        Should have keys: 'sets', 'parameters'
    solver : str, optional
        Solver to use ('gurobi' or other supported solvers). Default is 'gurobi'.
    verbose : bool, optional
        Whether to print progress messages. Default is True.
    enable_output : bool, optional
        Whether to enable solver output during optimization. Default is True.
    timing : bool, optional
        Whether to measure and report timing. Default is True.
    ...
    dict
        ...
        - 'variables': Dict of variable dictionaries (F, F_t, Storage_*, etc.)
        - 'status': Termination status
        - 'objective': Objective value (if optimal)
        - 'solution': Solution values for key variables (if optimal)
        - 'timing': Dict with timing information (if timing=True)
    """
    if verbose:
        print("="*70)
        print("Building PyOptInterface full model")
        print("="*70)
    ...
    # Create model
    if verbose:
        print("\n[1/2] Building model (variables + constraints)...")
    ...
    result = {
        ...
        'variables': {
            ...
            'Share_mobility_public': Share_mobility_public,
            'Share_freight_train': Share_freight_train,
            'Share_freight_road': Share_freight_road,
            'Share_freight_boat': Share_freight_boat,
            'Share_heat_dhn': Share_heat_dhn,
            'TotalCost': TotalCost,
            'TotalGWP': TotalGWP
        },
        'status': term_status
    ...
    if term_status == poi.TerminationStatusCode.OPTIMAL:
        ...
        solution['Share_heat_dhn'] = model.get_value(Share_heat_dhn)
        solution['Share_mobility_public'] = model.get_value(Share_mobility_public)
        solution['Share_freight_train'] = model.get_value(Share_freight_train)
        solution['Share_freight_road'] = model.get_value(Share_freight_road)
        solution['Share_freight_boat'] = model.get_value(Share_freight_boat)
        ...
        if verbose:
            ...
            print(f"    Share_heat_dhn: {solution['Share_heat_dhn']:.4f}")
            print(f"    Share_mobility_public: {solution['Share_mobility_public']:.4f}")
            print(f"    Share_freight_train: {solution['Share_freight_train']:.4f}")
            print(f"    Share_freight_road: {solution['Share_freight_road']:.4f}")
            print(f"    Share_freight_boat: {solution['Share_freight_boat']:.4f}")
            print("\n✓ PyOptInterface full model solved successfully.")
    else:
        if verbose:
...
if __name__ == "__main__":
    ...
    print("Running full model in standalone mode...")
    print("\n[Loading data...]")
    ...
    result = build_full_model(data, solver='gurobi', verbose=True, enable_output=True, timing=True)
